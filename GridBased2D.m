classdef GridBased2D < handle
    ...
    methods
        % class constructor
        function this = GridBased2D(rand_seed, max_nodes, map, conf)
            rng(rand_seed);
            ...
            this.rob = [this.half_width this.half_length; ...
                this.half_width -this.half_length; ...
                -this.half_width -this.half_length; ...
                -this.half_width this.half_length];
            this.scaled_rob = ceil([this.half_width/2 this.half_length/2; ...
                this.half_width/2 -this.half_length/2; ...
                -this.half_width/2 -this.half_length/2; ...
                -this.half_width/2 this.half_length/2]);
            this.rob_rect = ones(2*this.half_length, 2*this.half_width, 'int8');
        ...
        function position = steer(this, nearest_node, new_node_position)
            ...
            if abs(this.tree(3, nearest_node) - new_node_position(3)) > this.turn_theta
                position(3) = this.tree(3, nearest_node) + this.turn_theta;
            else
        ...
        function collision = obstacle_collision(this, new_node_position, node_index)
            ...
            rot_mat = [cosd(new_node_position(3)) -sind(new_node_position(3)); ...
                       sind(new_node_position(3)) cosd(new_node_position(3))];
            rob_c = int32(this.scaled_rob * rot_mat);
            temp0 = [ rob_c(:,1) + new_node_position(1) , rob_c(:,2) + new_node_position(2)];
            
            if any(temp0(:,1) > this.XY_BOUNDARY(2)) || any(temp0(:,1) < this.XY_BOUNDARY(1)) || ...
                    any(temp0(:,2) > this.XY_BOUNDARY(4)) || any(temp0(:,2) < this.XY_BOUNDARY(3))
                collision = true;
                return;
            end
            
            x_pos = double([max(rob_c(:,1)) min(rob_c(:,1))]);
            y_pos = double([max(rob_c(:,2)) min(rob_c(:,2))]);
            
            x_ind = x_pos + new_node_position(1);
            y_ind = y_pos + new_node_position(2);
            
            bw = poly2mask( double(rob_c(:,1)-x_pos(2)), double(rob_c(:,2)-y_pos(2)), y_pos(1)-y_pos(2) + 1,x_pos(1)-x_pos(2)+1);            
            diff_mat = this.map(x_ind(2):x_ind(1),y_ind(2):y_ind(1)) + uint8(bw)';
            
            if any(diff_mat(:) > 1)
                rob_c = int32(this.rob * rot_mat);
                temp0 = [ rob_c(:,1) + new_node_position(1) , rob_c(:,2) + new_node_position(2)];
        ...
        function rewire(this, new_node_ind, neighbors, min_node_ind)
            ...
            for ind = 1:numel(neighbors)
                ...
                if temp_cost < this.cumcost(neighbors(ind)) %&& ...
                    ...
                    % step is not so big we don't have to check for
                    % collisions
        ...
        function plot(this)
            ...
            for ind=numel(backtrace_path):-1:1
                subplot(1,2,2);
                rob_c = imrotate(this.rob_rect, this.tree(3,backtrace_path(ind)));
                temp1 = int32(floor(size(rob_c,1)/2));
                temp2 = int32(floor(size(rob_c,2)/2));
                ...
                temp_map = this.map;
                if(numel(y_pos(1):y_pos(2)) - size(rob_c,2) == 1) && (numel(x_pos(1):x_pos(2)) - size(rob_c,1) == 1)
                    rob_c = [rob_c zeros(numel(x_pos(1):x_pos(2))-1,1)];
                    rob_c = [rob_c ;zeros(1,numel(y_pos(1):y_pos(2)))];
                end
                temp_map(y_pos(1):y_pos(2), x_pos(1):x_pos(2)) = rob_c'*2;
                imshow(temp_map*50);
                pause(0.1);
                drawnow;
            end
